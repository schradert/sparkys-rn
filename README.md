...
- **Docs:** typedoc — `bun run docs` renders HTML into `docs/api` (git-ignored;
  regenerate locally). Exported-symbol TSDoc is lint-enforced (eslint-plugin-jsdoc).
- **Security:** `security` runs dependency-CVE (bun audit, osv-scanner),
  Nix-toolchain CVE (vulnix), and secret (gitleaks) scans.
- **AI skills:** `.claude/skills/` holds Claude Code playbooks — `onboarding`,
  `dev-workflow`, `eas-release`, `generate-docs`, `security-scan`,
  `ci-workflows`, and `upgrade-expo-sdk`.
